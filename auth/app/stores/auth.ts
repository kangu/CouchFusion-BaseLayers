...
export const useAuthStore = defineStore('auth', {
    ...
    getters: {
        ...
        userFullNameDirectory: (state): Record<string, string> => {
            ...
            if (currentUser?.name) {
                const profile = typeof currentUser.profile === 'object' ? currentUser.profile : {}
                const fullName = profile?.full_name || profile?.fullName || currentUser.full_name
    ...
    actions: {
        ...
        async fetchUser(): Promise<boolean> {
            ...
            try {
                ...
                if (error?.statusCode !== 401) {
                    this.error = error?.statusMessage || error?.message || 'Authentication check failed'
        ...
        async fetchAdminUsers(options: { force?: boolean } = {}): Promise<boolean> {
            ...
            try {
                ...
                this.adminUsers.total = typeof response?.total === 'number' ? response.total : list.length
                this.adminUsers.prefix = typeof response?.prefix === 'string' ? response.prefix : ''
                ...
                this.adminUsers.error = error?.statusMessage || error?.message || 'Failed to load users'
         ...
         */
        setLoggedUser(user/*: AuthUser | null, session: AuthSession | null*/): void {
            this.user = user
