<template>
  <div class="image-field">
    <div class="image-field__preview" v-if="previewUrl">
      ...
      <button type="button" class="image-field__clear" @click="clearImage" :disabled="pending">
        Remove
      </button>
    </div>
    ...
    <div class="image-field__controls">
      ...
      <div class="image-field__actions">
        <button type="button" class="image-field__button" @click="openLibrary" :disabled="pending">
          Browse
        </button>
        <button type="button" class="image-field__button" @click="triggerUpload" :disabled="pending">
          Upload
        </button>
        <button
          type="button"
    ...
    <Transition name="fade">
      <div v-if="isLibraryOpen" class="image-field__library-backdrop" @click.self="closeLibrary">
        <div class="image-field__library">
          <header class="image-field__library-header">
            <h3>Select Image</h3>
            <button type="button" class="image-field__close" @click="closeLibrary">
              ✕
            </button>
          </header>
          <div class="image-field__library-controls">
            ...
            />
            <button type="button" @click="searchLibrary" :disabled="isLibraryLoading">
              Search
            </button>
          </div>
          <div v-if="displayedCountLabel" class="image-field__library-summary">
            <span class="image-field__library-count">{{ displayedCountLabel }}</span>
            <button
              v-if="canShowAll"
              type="button"
          ...
          <div class="image-field__library-body">
            <p v-if="libraryError" class="image-field__error">{{ libraryError }}</p>
            <p v-else-if="isLibraryLoading" class="image-field__hint">Loading images…</p>
            <p v-else-if="libraryItems.length === 0" class="image-field__hint">
              No images found. Try uploading a new file.
            </p>
            <ul v-else class="image-field__library-grid">
              <li v-for="item in libraryItems" :key="item.fileId ?? item.filePath" class="image-field__library-card">
                <div class="image-field__library-thumbnail">
                  <img :src="item.thumbnailUrl || item.url || buildPreviewUrl(item.filePath)" alt="" loading="lazy" />
                </div>
                <div class="image-field__library-meta">
                  <strong class="image-field__library-name">{{ item.name || item.filePath }}</strong>
                  <small class="image-field__library-path">{{ item.filePath }}</small>
                </div>
                <button type="button" class="image-field__button image-field__button--primary" @click="selectFromLibrary(item)">
                  Use Image
                </button>
              </li>
            </ul>
...
import type { ComponentArrayItemField, ComponentPropSchema } from '#content/app/types/builder'
import type { ImageKitFile, ImageKitUploadResult } from '#imagekit/utils/imagekit'
...
const folderHint = computed(() => {
  const candidate = props.propDefinition?.ui && typeof props.propDefinition.ui === 'object'
    ? (props.propDefinition.ui as Record<string, unknown>).folder
    : undefined
  return typeof candidate === 'string' && candidate.trim() ? candidate.trim() : 'content-editor'
})

const placeholder = computed(() => props.propDefinition?.description || 'Enter ImageKit file path')

const urlEndpoint = computed(() => runtimeConfig.public?.imagekit?.urlEndpoint || '')
...
const buildPreviewUrl = (filePath?: string) => {
  ...
  const resolved = resolveImageKitUrl(filePath, urlEndpoint.value || undefined)
  return resolved
}
...
const fetchLibrary = async (override: { limit?: number } = {}) => {
  const requestedLimit = override.limit ?? libraryLimit.value ?? DEFAULT_LIBRARY_LIMIT
  ...
  try {
    ...
    libraryError.value =
      libraryFetchError instanceof Error ? libraryFetchError.message : 'Failed to load images'
  } finally {
...
const showAllImages = async () => {
  ...
  const targetLimit = libraryTotal.value > 0 ? Math.min(MAX_LIBRARY_LIMIT, libraryTotal.value) : MAX_LIBRARY_LIMIT
...
const selectFromLibrary = (item: ImageKitFile | ImageKitUploadResult) => {
  const rawValue = item.url ?? (typeof item.filePath === 'string' ? item.filePath : undefined)
  if (!rawValue) {
    return
  }
...
const copyToClipboard = async () => {
  ...
  try {
    ...
    error.value =
      clipboardError instanceof Error ? clipboardError.message : 'Unable to copy to clipboard'
  }
}
