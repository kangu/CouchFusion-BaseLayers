...
import { contentToMinimalDocument, minimalToContentDocument } from '#content/utils/page-documents'
import type {
  ContentPageSummary,
...
const defaultUi: UiConfig = {
  createButton: 'inline-flex items-center gap-2 rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2',
  pageChipActive: 'border-blue-500 bg-blue-50 text-blue-600 shadow-sm',
  pageChipInactive: 'border-gray-200 bg-white text-gray-700 hover:border-blue-300 hover:text-blue-600',
  saveButton: 'inline-flex items-center gap-2 rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-70',
  deleteButton: 'inline-flex items-center gap-2 rounded-md border border-red-200 px-4 py-2 text-sm font-semibold text-red-600 hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-70',
  cancelButton: 'inline-flex items-center gap-2 rounded-md border border-gray-300 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-2',
  modalSaveButton: 'inline-flex items-center gap-2 rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-70',
  modalCancelButton: 'rounded-md border border-gray-300 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50'
}
...
const description = computed(() => props.description ?? 'Manage content pages and edit their layout using the builder workbench.')
...
const filteredPages = computed(() => {
  ...
  return list.filter((page) => {
    ...
    return titleValue.includes(normalizedFilter.value) || pathValue.includes(normalizedFilter.value)
  })
})
...
const historyEntries = computed<ContentPageHistoryEntry[]>(() => historyState.value?.data ?? [])
...
const selectedHistoryDocument = computed<MinimalContentDocument | null>(() => {
  ...
  const entry = historyEntries.value.find((item) => item.id === selectedHistoryId.value)
  if (!entry) {
    return null
  }
...
const selectedDocument = computed<MinimalContentDocument | null>(() => {
  return selectedHistoryDocument.value ?? resolveDocument(selectedSummary.value)
})
...
onMounted(() => {
  ...
  if (headerSentinelRef.value) {
    headerObserver = new IntersectionObserver(handleIntersection, { threshold: 0 })
...
function resolveDocument(summary: ContentPageSummary | null | undefined): MinimalContentDocument | null {
  if (!summary) {
    return null
  }
...
async function handleCreatePage(): Promise<void> {
  ...
  try {
    ...
    const wrapped = error instanceof Error ? error : new Error(error?.message || 'Failed to create page.')
...
async function handleSaveDocument(): Promise<void> {
  ...
  try {
    ...
    if (selectedSummary.value.document) {
      ...
      contentDocument.createdAt = selectedSummary.value.document.createdAt ?? null
    }
    ...
    feedback.value.success?.(`Page "${savedSummary.title}" saved successfully`, {
      label: 'View Page',
      href: savedSummary.path
    })
  } catch (error: any) {
    const wrapped = error instanceof Error ? error : new Error(error?.message || 'Failed to save page.')
    saveError.value = wrapped.message
    emit('save-error', wrapped)
    feedback.value.error?.(wrapped.message)
  } finally {
...
async function handleDeletePage(): Promise<void> {
  ...
  if (props.confirmDelete) {
    ...
    confirmed = window.confirm(`Delete page "${label}"? This action cannot be undone.`)
  }
  ...
  try {
    ...
    const wrapped = error instanceof Error ? error : new Error(error?.message || 'Failed to delete page.')
...
<template>
  <div class="content-admin-workbench space-y-6">
    ...
    <div class="content-admin-workbench__panel">
      ...
      <div class="content-admin-workbench__chip-list">
        <template v-if="isIndexLoading">
          <span class="content-admin-workbench__hint">Loading pages…</span>
        </template>
        <template v-else-if="isFilteredEmpty">
          <span class="content-admin-workbench__hint">No pages match your filter.</span>
        </template>
        <template v-else-if="!hasPages">
          <span class="content-admin-workbench__hint">No pages available yet. Create one to get started.</span>
        </template>
        <template v-else>
          <button
            ...
            :class="page.path === selectedPath ? ui.pageChipActive : ui.pageChipInactive"
    ...
    <div ref="editorCardRef" class="content-admin-workbench__editor">
      <div ref="headerSentinelRef" aria-hidden="true" class="content-admin-workbench__sentinel"></div>
      ...
      >
        <div class="editor-header__left">
          <div class="editor-header__meta">
            ...
            <span class="editor-header__meta-value">
              {{ selectedSummary?.title || selectedSummary?.path || 'No page selected' }}
            </span>
          </div>
          <div class="editor-header__status">
            <span v-if="selectionError" class="editor-header__status-error">{{ selectionError }}</span>
            <span v-else-if="lastUpdatedDisplay" class="editor-header__status-time">
        ...
        <div class="editor-header__actions">
          ...
          >
            <Icon v-if="isSavePending" name="mdi:loading" class="h-4 w-4 animate-spin" />
      ...
      <div class="content-admin-workbench__editor-body">
        <div class="content-admin-workbench__editor-sidebar">
          ...
          <div class="sidebar__history">
            ...
            <div v-if="isHistoryLoading" class="sidebar__hint">Loading history…</div>
            <div v-else-if="historyEntries.length === 0" class="sidebar__hint">No history available yet.</div>
            <template v-else>
              <button
                ...
                type="button"
                class="sidebar__history-item"
                :class="{ 'is-active': entry.id === selectedHistoryId }"
                @click="handleSelectHistory(entry.id)"
              >
                <Icon name="mdi:backup-restore" class="h-4 w-4" />
                <span>{{ formatHistoryLabel(entry.timestamp) }}</span>
              </button>
            </template>
          </div>
        </div>
        ...
        <div class="content-admin-workbench__editor-canvas">
          ...
          <div v-if="selectedDocument" class="editor-canvas__workbench">
            <BuilderWorkbench
              ref="builderRef"
          ...
          <div class="modal__panel" role="dialog" aria-modal="true">
            <div class="modal__header">
              <div>
                ...
                <p class="modal__subtitle">Provide the basic page metadata to bootstrap the builder.</p>
              </div>
              <button
                type="button"
                class="modal__close"
            ...
            <form class="modal__form" @submit.prevent="handleCreatePage">
              ...
              <div class="modal__field-grid">
                <div class="modal__field">
                  <label for="new-page-seo-title">SEO title</label>
                  <input
                ...
                <div class="modal__field">
                  <label for="new-page-seo-description">SEO description</label>
                  <input
              ...
              <div class="modal__field">
                <label for="new-page-meta">Meta JSON (optional)</label>
                <textarea
                  id="new-page-meta"
              ...
              <div class="modal__actions">
                ...
                >
                  <Icon v-if="isCreatingPage" name="mdi:loading" class="h-4 w-4 animate-spin" />
                  <span>{{ isCreatingPage ? 'Saving…' : 'Save Page' }}</span>
                </button>
              </div>
            </form>
          </div>
