...
export default defineEventHandler(async (event) => {
    ...
    try {
        ...
        console.log('normalized pathm', normalizedPath)
        const documentId = contentIdFromPath(normalizedPath)
        console.log('documentId', documentId)
        const databaseName = getContentDatabaseName()

        const existing = await getDocument<Record<string, any>>(databaseName, documentId)
        ...
        const response = await deleteDocument(databaseName, existing._id, existing._rev)
