...
- Added a new `content` layer that extends the shared database utilities, validates `dbContentPrefix`, and exposes composables and stores through Nuxt auto-imports.
- Introduced a CouchDB initialization plugin that provisions `${dbContentPrefix}-content` and registers the `_design/content` design document with a `by_path` view to query pages by URL.
...
- Manual verification plan: run the consuming app with `npm run dev`, ensure CouchDB env vars plus `dbContentPrefix` are set, then call the `/api/content/pages` endpoints (list, `?path=`) and exercise the Pinia store actions via browser devtools or component usage.
