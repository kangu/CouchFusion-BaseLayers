...
- The new `module/ignored-prefixes.ts` runs during Nuxt setup, inspects the consuming app's `/pages` directory, filters static entries, normalises the prefixes, and merges them with any `appConfig.content.manualIgnoredPrefixes`.
...
```ts
// layers/content/module/ignored-prefixes.ts
