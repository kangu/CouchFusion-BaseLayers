...
- `module/sitemap-routes.ts` scans the consuming app's `/pages` tree during init/build hooks, persists the deduplicated static paths, and exposes them through both runtime config and a build artifact for runtime consumption.
