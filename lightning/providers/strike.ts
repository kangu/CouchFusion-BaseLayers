...
export function createStrikeProvider(config: StrikeConfig): LightningProvider {
  ...
  const createInvoice = async (
    ...
    const quoteResponse = await makeRequest(
      `/v1/invoices/${invoiceResponse.invoiceId}/quote`,
      {
        method: "POST",
      },
    );
    ...
    return {
      ...
      sourceAmount: quoteResponse.sourceAmount
        ? parseFloat(quoteResponse.sourceAmount)
        : undefined,
      targetAmount: quoteResponse.targetAmount
        ? parseFloat(quoteResponse.targetAmount)
        : undefined,
    };
  };
  ...
  const getInvoiceById = async (invoiceId: string): Promise<any> => {
    try {
      console.log(
        "🔍 Fetching current invoice state from Strike API:",
        invoiceId,
      );
  ...
  const processWebhook = async (payload: any): Promise<WebhookEvent | null> => {
    ...
    if (
      payload.eventType === "invoice.updated" &&
      payload.data.changes?.includes("state")
    ) {
      console.log(
        "📋 State change detected for invoice:",
      ...
      try {
        ...
        const webhookEvent: WebhookEvent = {
          ...
          amount: currentInvoice.amount
            ? parseFloat(currentInvoice.amount.amount)
            : undefined,
  ...
  const setupWebhookSubscription = async (webhookUrl: string): Promise<any> => {
    ...
    try {
      ...
      const matchingSubscription = existingSubscriptions.find(
        (sub: any) => sub.webhookUrl === webhookUrl && sub.enabled === true,
      );

      if (matchingSubscription) {
        console.log(
          "✅ Found existing Strike webhook subscription for URL:",
          webhookUrl,
        );
        ...
        if (hasExpectedEvents) {
          console.log(
            "✅ Existing subscription has all required event types, skipping creation",
          );
          return matchingSubscription;
        } else {
          console.log(
            "⚠️ Existing subscription missing required event types, will create new one",
          );
        }
      } else {
        console.log(
          "🔄 No existing subscription found, creating new one for:",
          webhookUrl,
        );
      }
    } catch (listError) {
      console.warn(
        "⚠️ Failed to list existing subscriptions, proceeding with creation:",
        listError.message,
      );
    ...
    const payload = {
      ...
      enabled: true,
      eventTypes: ["invoice.created", "invoice.updated"],
    };

    console.log("Setting up Strike webhook subscription:", {
      webhookUrl,
