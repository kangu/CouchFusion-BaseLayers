...
async function initializeStrikeWebhook(runtimeConfig: any): Promise<void> {
    ...
    try {
        ...
        if (!process.env.PUBLIC_SITE_URL) {
            console.warn('⚠️ PUBLIC_SITE_URL not set, skipping Strike webhook subscription setup');
            return;
        }
        ...
        if (!lightningConfig.providers?.strike) {
            console.warn('⚠️ Strike provider configuration not found, skipping webhook subscription setup');
            return;
        }
        ...
        if (!strikeConfig.webhookSecret) {
            console.warn('⚠️ Strike webhook secret not configured, skipping webhook subscription setup');
            return;
        }
        ...
        if (strikeProvider.setupWebhookSubscription) {
            const webhookUrl = `${process.env.PUBLIC_SITE_URL}/api/webhooks/strike`;
        ...
        console.error('💥 Strike webhook subscription initialization failed:', error);
...
async function initializeLightningLayer(): Promise<void> {
    ...
    try {
        ...
        console.log('🎉 CouchDB lightning layer initialization completed successfully');
